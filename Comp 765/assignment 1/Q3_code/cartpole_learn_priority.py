...
#will hold priority of each state
priority = np.multiply(np.ones((number_x,number_v,number_av,number_a)),-100000)
...
def state_score(s):
    s[3] -= np.pi
    #print(Q)
    #return -np.dot(np.dot(s, Q),np.transpose(s))
    return 1.0/(np.dot(np.dot(s, Q),np.transpose(s))+0.0001)
...
def value_iteration(plant):
    ...
    if plant.noise is not None:
        ...
        goal_score = state_score(goal)

        priority[goal_index] = goal_score

        #specify number of iterations
        for i in range(10000):
            #get the index of the state with highest priority
            index = np.unravel_index(priority.argmax(), priority.shape)
            #state to index
            state = index_to_state(index)
            #look ahead
            A = one_step_lookahead(index,V,plant)
            #get best action value
            V_new = np.max(A)
            #compute the value change
            delta_i = np.abs(V_new - V[index])
            #print(delta_i)
            #update value of index
            V[index] = V_new

            #get neighbours
            neighbours = get_neighbour_states(state)
            #get neighbours probability
            p_neighbours = neighbour_prob(state,neighbours)

            for j in range(16):

                new_index = state_to_index(neighbours[j])
                #if its valid it will have a spot in priority
                if index_valid(new_index):
                    #Use delta to modify the priorities of the predecessors of s.
                    new_priority = delta_i * p_neighbours[j]
                    #get current priority
                    old_priority = priority[new_index]
                    #if new priority is higher

                    if new_priority > old_priority:
                        priority[new_index] = new_priority
...
def main():
    ...
    for i in range(N):
        ...
        plt.show()
        #plt.savefig('plots_sweeping/plot_'+str(i))
