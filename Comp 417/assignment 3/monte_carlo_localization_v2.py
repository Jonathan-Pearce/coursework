...
class ParticleFilter(object):
    ...
    def handle_observation(self, laser_scan, dt):
        ...
        for i in self.particles:
            self.predict_particle_odometry(i)
            error = (self.get_prediction_error_squared(laser_scan, i))
            errors.append(error)
        ...
        for i in range(len(errors)):
            new_weights.append(math.exp(-1*math.pow(errors[i],1)/math.pow(self.beam_range_measurement_noise_std_dev,2))) 
        ...
        if weightSum != 0:
            ...
            for i in range(len(self.weights)):
                self.weights[i] = 1/len(self.particles)
    ...
    def resample(self):
        ...
        self.new_particles = []
        #probabalistic factor
        ##Can be used to set how many random particles you want to generate
        ##Note: Did not end up using probalistic factor
        p = 1
        length = len(self.particles)
        N = (int)(len(self.particles) * p)
        r = random.uniform(0,1/N)
        c = self.weights[0]
        index = 0

        #Resample N new particles from existing particles
        for n in range(N):
            u = r + (n+1)*1.0/N
            ...
            newParticle = Particle(self.particles[index].id, self.particles[index].x, self.particles[index].y, self.particles[index].theta)
            self.new_particles.append(newParticle)

        #This is the code for generating random particles
        #get average theta of all particles
        thetaSum = 0
        for i in range(len(self.new_particles)):
            thetaSum += self.new_particles[i].theta
        avgTheta = thetaSum/len(self.new_particles)
        #generate random particles
        for n in range(length - N):
            xrand, yrand, theta = self.get_random_free_state()
            self.new_particles.append(Particle(50, xrand, yrand, avgTheta))
        ...
        for angle in angles:
            ...
            while r <= max_range:
                ...
                if xm > self.xmax or xm < self.xmin:
                    #added from advice on myCourses
                    r = max_range
                    break
    ...
    def get_prediction_error_squared(self, laser_scan_msg, particle):
        ...
        ##get 2 arrays from subsample laser scan
        robotRanges, scanAngles = self.subsample_laser_scan(laser_scan_msg)
        
        # TODO: simulate a laser scan using one of the methods of this class

        ## use (x,y,theta), angles from previous method, use laser min and max ranges
        particleRanges = self.simulate_laser_scan_for_particle(particle.x, particle.y, particle.theta, scanAngles, self.laser_min_range, self.laser_max_range)
        
        # TODO: compute the difference bwteen predicted ranges and actual ranges

        # Take the squared norm of that difference
        norm_error = 0
        #sum the squared differences of each laser scan
        for i in range(len(robotRanges)):
            norm_error += math.pow(robotRanges[i] - particleRanges[i], 2)
        norm_error = math.sqrt(norm_error)  
        return norm_error**2
    ...
    def metric_to_grid_coords(self, x, y):
        ...
        #added -1 to these lines as suggest on MyCourses by classmate
        row = min(max(int(gy), 0), self.ogm.info.height-1)
        col = min(max(int(gx), 0), self.ogm.info.width-1)
        return (row, col)
...
if __name__ == '__main__':
    ...
    #changed to 10 based on advice on myCourses from classmate
    xmin = -10
    xmax = 10
    ymin = -10
    ymax = 10
