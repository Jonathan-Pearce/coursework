...
def genPoint():
    ...
    #generates a random point on grid
...
# Implementation at bottom of method
def rrt_search(G, tx, ty):
    ...
    if visualize:
    	canvas.markit(start_x, start_y, r=10)
    	canvas.markit(tx, ty, r=10)
    n=0
    totalDistance = 0
    rrt_iterations = 0
    ...
    while True:
        ...
        randPoint = genPoint()
       	...
       	#sampled point has to be inframe
        if randPoint[0] < 0 or randPoint[0] > XMAX or randPoint[1] < 0 or randPoint[1] > YMAX:
        	continue
    ...
    while n != 0:
        ...
        prevN = n
        n = returnParent(n)
        totalDistance += pointPointDistance(vertices[n], vertices[prevN])
        if visualize:
        	canvas.events()

    #print totalDistance
    if not visualize:
    	return [rrt_iterations, totalDistance]
...
if __name__ == '__main__':
    ...
    stepLengths = [4,8,12,20,30,50,75,125,250,500]
    for i in range(len(stepLengths)):
	    SMALLSTEP = stepLengths[i]
	    print SMALLSTEP
	    iterationsList = []
	    pathLengthList = []
	    for j in range(0,10):
	    	G = [[0], []]
	        data = rrt_search(G,tx,ty) 
	        iterationsList.append(data[0])
	        pathLengthList.append(data[1])

	    print iterationsList
	    print pathLengthList
	    text_file.write(str(SMALLSTEP))
	    text_file.write('\n')
	    text_file.write(",".join(str(x) for x in iterationsList))
	    text_file.write('\n')
	    text_file.write(",".join(str(x) for x in pathLengthList))
	    text_file.write('\n')
