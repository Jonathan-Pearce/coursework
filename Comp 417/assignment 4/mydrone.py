...
def draw_objects():
    ...
    #added this line
    global grid, gridCount, totalGridCount, gridTypes, distance, gridPrev, random
    ...
    #My Code
    imTemp = np.asarray(im, dtype=np.float32).flatten()
    typeLand = (geoclass.classifyOne(pca, clf, imTemp, classnames))
    name = typeLand[1]
    typeLand = typeLand[0]
    print typeLand
    print name
    ...
    if random == 0:
	    #figure out what grid your in
	    oldgridX = int((oldp[0]-tx) / 51.2)-1
	    oldgridY = int((oldp[1]-ty)/ 51.2)-1
	    gridX = int(oldp[0] / 51.2)-1
	    gridY = int(oldp[1] / 51.2)-1

	    #checks whether we are in new square and updates our simple grid
	    print gridX,' ',gridY,' ',oldp,' ',tx,' ',ty
	    if oldgridX != gridX or oldgridY != gridY:
	    	if grid[gridX][gridY] == -1:
	    		grid[gridX][gridY] = 1
	    		gridCount += 1
	    	totalGridCount += 1
	    	gridTypes[typeLand] += 1
	    ...
	    if typeLand == 2:
		    ...
		    growthFactor = 50
		    tx = (random.random()*2 - 1)*growthFactor
		    ty = (random.random()*2 - 1)*growthFactor

		    #prevent from leaving field
	    if oldp[0]+tx >= 1024:
	        tx = -50
	    elif oldp[0]+tx <= 0:
		    tx = 50
	...
	#DFS
    	...
    	#get your location
	    gridX = int(oldp[0] / 51.2)
	    gridY = int(oldp[1] / 51.2)
	    ...
	    if typeLand == 2:
	        ...
	        tx *= -1
	        ty *= -1
	    #else, look around you
	    else:
	        if grid[gridX][gridY] == -1:
		        gridCount += 1
		        grid[gridX][gridY] = typeLand
		        gridPrev[gridX][gridY] = [-int(tx/51.2), -int(ty/51.2)]
		      #we have room to look right and its untravelled then lets go!
	        if gridX < 19 and grid[gridX+1][gridY] == -1:
	            tx = 51.2
	            ty = 0
	        elif gridY < 19 and grid[gridX][gridY+1] == -1:
	            print grid[gridX][gridY+1]
	            tx = 0
	            ty = 51.2
	        elif gridX > 0 and grid[gridX-1][gridY] == -1:
	            tx = -51.2
	            ty = 0
	        elif gridY > 0 and grid[gridX][gridY-1] == -1:
	            tx = 0
	            ty = -51.2
	        #we got nowhere to go! backtrack
	        else:
	            tx = gridPrev[gridX][gridY][0] * 51.2
	            ty = gridPrev[gridX][gridY][1] * 51.2
	         

    distance += math.sqrt(tx*tx + ty*ty)

    print 'Number of unique grid squares: ', gridCount
    print 'Number of total grid squares: ', totalGridCount
    print 'Grid Type Counts: ', gridTypes
...
#Adding this line
grid = [[-1 for x in range(20)] for y in range(20)]
gridPrev = [[-1 for x in range(20)] for y in range(20)]
gridCount = 0
totalGridCount = 0
gridTypes = [0,0,0,0]
distance = 0
#variable to decide which Algorithm to use
random = 1
